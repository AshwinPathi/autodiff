...
class Matrix : public Tensor<T> {
   public:
    ...
    Matrix(size_t size_dim1, size_t size_dim2, const std::vector<T>& buffer)
        : Tensor<T>{{size_dim1, size_dim2}, buffer} {}
    Matrix(size_t size_dim1, size_t size_dim2, std::vector<T>&& buffer)
        : Tensor<T>{{size_dim1, size_dim2}, std::move(buffer)} {}

    // 2-d buffer construction from "canonical" 2d vector
    Matrix(const std::vector<std::vector<T>>& buffer)
        : Tensor<T>{{buffer.size(), buffer[0].size()}, flatten(buffer)} {}

    size_t row_dim() const { return this->shape_[0]; }

    size_t col_dim() const { return this->shape_[1]; }
