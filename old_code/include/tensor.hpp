...
// Very basic (and naive) implementation of a tensor. Note that the dimensions
// are dynamic, so this means there are no compile-time nicities with verifying
// matrix multiplication shapes, etc.
template <typename T>
...
class Tensor {
   public:
    ...
    //////// Constructors
    // Default constructed Tensor shouldn't really be used for anything apart
    // from being a staging variable for storing dummy tensors.
    Tensor() = default;
    ...
    Tensor(const std::vector<size_t>& shape, bool zeros) : shape_{shape} {
        ...
        // the underlying buffer is really just a 1-d vector thats strided differently
        // based on the shape.
    ...
    Tensor(const std::vector<size_t>& shape, const std::vector<T>& buffer)
        : shape_{shape}, buffer_{buffer} {}
    ...
    Tensor(Tensor<T>&& other)
        : shape_{std::move(other.shape_)}, buffer_{std::move(other.buffer_)} {}
    ...
    const data_type& operator[](size_t index) const { return buffer_[index]; }

    data_type& operator[](const index_type& indices) { return buffer_[linearize_index(indices)]; }

    data_type& operator[](size_t index) { return buffer_[index]; }
